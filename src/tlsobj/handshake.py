...
class Handshake(object):
    ...
    def __init__(self, chello=None, serverdata=None, certificatedata=None,
                 ...
                 size=None, begintime=None, time=None, encrypted=None):
    ...
    def iscomplete(self):
        for attr, value in self.__dict__.items():
            if value is None and attr != 'certificatedata':
                return 0
        return 1
    ...
    def hasKEXandAuthData(self):
        if (self.chello is not None) and (self.serverdata is not None) \
                and (self.certificatedata is not None or self.encrypted is not None) and (self.certificateverify is not None) \
    ...
    def setSize(self):
        ...
        certsize = self.certificatedata.certsLength if self.certificatedata is not None else self.encrypted.size
        self.hssize = self.chello.size + self.serverdata.size + certsize + \
            self.certificateverify.signatureLength + self.finished.size
